...
class LocalAggregator_mix(nn.Module):
    ...
    def forward(self, hidden, adj, mask_item=None):
        ...
        a_input = torch.cat([h.repeat(1, 1, N).view(batch_size, N * N, self.dim), h.repeat(1, N, 1)], -1).view(
            batch_size, N, N, 2 * self.dim)
...
class GNN(Module):
    def __init__(self, hidden_size, step=2):
        ...
        self.linear_in1 = nn.Linear(self.gate_size, self.gate_size,
                                    bias=True)
        self.linear_in2 = nn.Linear(self.gate_size, self.gate_size, bias=True)

    def GNNCell(self, hidden, A):
